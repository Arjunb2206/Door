...
window.sendChatMsg = function() {
  ...
  setTimeout(() => {
    const replies = ['Understood, I\'ll be there shortly.', 'Can you share the exact location?', 'I\'m on my way.', 'Please stay calm, help is coming.', 'Coordinating with the medical team now.'];
    area.insertAdjacentHTML('beforeend', `<div class="msg recv">${replies[Math.floor(Math.random()*replies.length)]}<div class="msg-time">${t}</div></div>`);
    area.scrollTop = area.scrollHeight;
  }, 1200);
};
